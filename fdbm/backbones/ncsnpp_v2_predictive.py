...
@BackboneRegistry.register("ncsnpp_v2_predictive")
class NCSNpp_v2_predictive(nn.Module):
    ...
    def __init__(
        ...
        **unused_kwargs
        ...
        in_channels = 2   # x.real, x.imag, y.real, y.imag
    ...
    def forward(self, x):
        ...
        h = modules[m_idx](h)  # ResNet block
        m_idx += 1
        ...
        if y.shape[2] == 257:
            h = torch.cat((h, torch.zeros_like(h[:, :, :1, :])), dim=2)

        return h


@BackboneRegistry.register("ncsnpp_v2_5M_predictive")
class NCSNpp_v2_5M_predictive(NCSNpp_v2_predictive):
...
if __name__ == "__main__":
    model = NCSNpp_v2_predictive().eval()
    x = torch.view_as_complex(torch.randn(1, 1, 257, 256, 2))
    out = model(x)
    print(out.shape)

    from ptflops import get_model_complexity_info

    class Mymodel(NCSNpp_v2_predictive):
        def __init__(self):
            super().__init__()
    ...
    flops, params = get_model_complexity_info(
        mymodel, (1, 257, 256), as_strings=True,
