...
class Corrector(abc.ABC):
    ...
    def __init__(self, bridge, model, snr, n_steps):
        super().__init__()
        self.bridge = bridge
        self.model = model
        self.snr = snr
...
class LangevinCorrector(Corrector):
    def __init__(self, bridge, model, snr, n_steps):
        super().__init__(bridge, model, snr, n_steps)
        self.n_steps = n_steps
        self.snr = snr
    ...
    def update_fn(self, x, y, t, *args):
        ...
        for _ in range(self.n_steps):
            s = self.model(x, y, t)
            grad = self.bridge.score_fn(t, x, s, y)
            noise = torch.randn_like(x)
            grad_norm = torch.norm(grad.reshape(grad.shape[0], -1), dim=-1).mean()
            noise_norm = torch.norm(noise.reshape(noise.shape[0], -1), dim=-1).mean()
            step_size = ((target_snr * noise_norm / (grad_norm + 1e-8)) ** 2 * 2).unsqueeze(0)
