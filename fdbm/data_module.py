...
from fdbm.util.other import read_scp
...
class Specs(Dataset):
    def __init__(self, data_dir, subset, dummy, shuffle_spec, num_frames,
            format='default', normalize="noisy", spec_transform=None, num_data_per_epoch=None,
            stft_kwargs=None, **ignored_kwargs):

        # Read file paths according to file naming format.
        if format == "default":
            self.clean_files_all = []
            self.clean_files_all += sorted(glob(join(data_dir, subset, "clean", "*.wav")))
            self.clean_files_all += sorted(glob(join(data_dir, subset, "clean", "**", "*.wav")))
            self.noisy_files_all = []
            self.noisy_files_all += sorted(glob(join(data_dir, subset, "noisy", "*.wav")))
            self.noisy_files_all += sorted(glob(join(data_dir, subset, "noisy", "**", "*.wav")))
        else:
    ...
    def sample_data_per_epoch(self):
        if self.num_data_per_epoch is None:
            self.clean_files = self.clean_files_all
            self.noisy_files = self.noisy_files_all
        else:
            indices = np.random.choice(len(self.clean_files_all), size=self.num_data_per_epoch, replace=False)
            self.clean_files = [self.clean_files_all[i] for i in indices]
            self.noisy_files = [self.noisy_files_all[i] for i in indices]
...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument("--normalize", type=str, choices=("clean", "noisy", "not", "std"), default="noisy", help="Normalize the input waveforms by the clean signal, the noisy signal, or not at all.")
        parser.add_argument("--transform_type", type=str, choices=("exponent", "log", "none", "exponent_diff"), default="exponent", help="Spectogram transformation for input representation.")
    ...
    def __init__(
        ...
        n_fft=510, hop_length=128, num_frames=256, window='hann', num_data_per_epoch=None,
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.train_set = Specs(data_dir=self.base_dir, subset='train',
                ...
                normalize=self.normalize, num_data_per_epoch=self.num_data_per_epoch, **specs_kwargs)
            self.valid_set = Specs(data_dir=self.base_dir, subset='valid',
                dummy=False, shuffle_spec=False, format=self.format,
